    ...
    if (!command.empty()) {
        ...
        if (cmd.size() > 0) {
            if (cmd[0] == "NEW_BULLET") {
                ...
                this->bullet.emplace_back(new Bullet(sf::Vector2f(std::stod(cmd[1]), std::stod(cmd[2])), cmd[3], 1150,
                                                     cmd[3] == "monster" ? -1 : 1));
                ...
                if (GetMonsterByIdFromServer(cmd[1]) != -1) {
                    std::cout << "clean monster: " << cmd[1] << " index: " << (GetMonsterByIdFromServer(cmd[1]))
                              << std::endl;
